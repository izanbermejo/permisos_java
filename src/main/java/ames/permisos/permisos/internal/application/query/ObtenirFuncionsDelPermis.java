package ames.permisos.permisos.internal.application.query;

import ames.permisos.organigrama.internal.domain.Funcio;
import ames.permisos.permisos.internal.infraestructure.PermisRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import java.util.List;

@Component
public class ObtenirFuncionsDelPermis {

    @Autowired
    PermisRepository permisRepo;

    public List<Funcio> executar(String nomAplicacio, String nomModul, String nomPermis) {
        return permisRepo.listFuncioDelPermis(nomAplicacio, nomModul, nomPermis);
    }
}