...
@Service
public class ExportarDetallComandesClient {
    private XSSFSheet sheet;
    private XSSFWorkbook workbook;
    CellStyle headerStyle, intStyle, decimalStyle, dateStyle;
    int rowNum;

    public byte[] exportar(String client, LocalDate dataInici, LocalDate dataFi, boolean mostrarEliminades) throws IOException {

        workbook = new XSSFWorkbook();
        sheet = workbook.createSheet("Detalls");
        var dades = BeanUtils.getBean(ObtenirDetallComandesClient.class).executar(client, dataInici, dataFi, mostrarEliminades);
    ...
    private void aplicaAmpladaColumnes(XSSFSheet sheet) {
        ...
        sheet.setColumnWidth(colNum++, 5500);   // tipus
    ...
    private void omplirDades(XSSFSheet sheet, List<ObtenirDetallComandesClient.ObtenirDetallComandesClientResponse> registres) {
        rowNum = 1;
        for (var r : registres) {
            var colNum = 0;
