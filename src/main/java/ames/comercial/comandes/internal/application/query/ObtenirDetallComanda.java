...
@Service
public class ObtenirDetallComanda {

    @Autowired @Qualifier("jdbcAmes")
    JdbcTemplate jdbcAmes;

    public List<ObtenirDetallComandaResponse> executar(int comanda, LocalDate dataInici, LocalDate dataFi, boolean mostrarEliminades) {
        var params = new MapSqlParameterSource();
        params.addValue("comanda", comanda, Types.INTEGER)
        ...
        return new NamedParameterJdbcTemplate(jdbcAmes).query("""
                        WITH ultima_linia AS (
                            SELECT DISTINCT ON (lc.comanda, lc.numero)
                                lc.comanda,
                                lc.numero,
                                lc.artint,
                                lc.clicod,
                                lc.tipus,
                                lc.data_solicitada,
                                lc.data_prevista_sortida,
                                lc.data_prevista_sortida_interna,
                                lc.data_confirmada_fabrica,
                                lc.quantitat,
                                lc.quantitat_servida,
                            ...
                            WHERE
                                lc.comanda = :comanda
                                AND lc.datareg_local <= :dataFinal
                            ORDER BY lc.comanda, lc.numero, lc.datareg DESC
                        )
                        SELECT
                            l.comanda,
                            l.numero,
                            c.informacio_client->>'identificador' AS identificador_client,
                            l.tipus,
                            ac.codi_fabrica,
                            ac.referencia,
                            l.data_solicitada,
                            l.data_prevista_sortida,
                            l.data_prevista_sortida_interna,
                            l.data_confirmada_fabrica,
                            l.quantitat,
                            l.quantitat_servida,
                            l.quantitat_pendent,
                            l.preu,
                            l.divisa,
                            com.intern AS "comIntern",
                            com.client AS "comClient"
                        FROM ultima_linia l
                        ...
                        ORDER BY
                            l.data_solicitada ASC,
                            l.comanda ASC,
                            l.numero ASC;
                """,
                params,
                this::mapAllDetallComanda
        );
    }

    private ObtenirDetallComandaResponse mapAllDetallComanda(ResultSet rs, int rowNum) throws SQLException {
        return ObtenirDetallComandaResponseImpl.builder()
    ...
    @JsonDeserialize(builder = ObtenirDetallComandaResponseImpl.Builder.class)
    @Value.Style(typeImmutable = "*Impl")
    @Value.Immutable
    public interface ObtenirDetallComandaResponse {
        long comandaAMES();
        long numero();
