...
import ames.comercial.comandes.internal.application.query.ObtenirDetallComanda;
import ames.comercial.comandes.internal.application.query.ObtenirDetallComanda.ObtenirDetallComandaResponse;
...
@Service
public class ExportarDetallComanda {
    private XSSFSheet sheet;
    private XSSFWorkbook workbook;
    CellStyle headerStyle, intStyle, decimalStyle, dateStyle;
    int rowNum;

    public byte[] exportar(int comanda, LocalDate dataInici, LocalDate dataFi, boolean mostrarEliminades) throws IOException {

        workbook = new XSSFWorkbook();
        sheet = workbook.createSheet("Detalls");
        var dades = BeanUtils.getBean(ObtenirDetallComanda.class).executar(comanda, dataInici, dataFi, mostrarEliminades);
    ...
    private void aplicaAmpladaColumnes(XSSFSheet sheet) {
        ...
        sheet.setColumnWidth(colNum++, 4000);   // tipus
        sheet.setColumnWidth(colNum++, 4000);   // codi fabrica
        sheet.setColumnWidth(colNum++, 6000);   // referència
    ...
    private void omplirDades(XSSFSheet sheet, List<ObtenirDetallComandaResponse> registres) {
        rowNum = 1;
        for (var r : registres) {
            var colNum = 0;
